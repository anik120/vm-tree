...
Usage:
    ./vm-tree.py <vm-name> [--namespace <ns>] [--all-namespaces]
    ./vm-tree.py --storage-class <sc-name>  # Show all VMs using a storage class
...
def run_oc(args: List[str], check=True) -> Dict[str, Any]:
    """Run oc command and return parsed JSON output"""
    try:
...
Examples:
  ...
  %(prog)s my-vm -n my-namespace
    ...
    try:
        ...
        print(f"{Colors.FAIL}Error: 'oc' command not found. Please install OpenShift CLI.{Colors.ENDC}")
        sys.exit(1)

    # Mode 1: Show storage class usage
